...
namespace {
struct GSLARGS {
    size_t indexP, indexCosTheta, indexPhi;
    double pMin, pMax;
...
inline double Expansion(std::array<double, 3> x, const GSLARGS& args) {
    ...
    return HatFunctionBasis::MomentumBasis(indexP, p) *
           ...
           HatFunctionBasis::PhiBasis(indexPhi, phi) *
...
std::vector<double> Compute() {
    ...
    size_t Nphi = HatFunctionBasis::PhiParams.Nx;
    std::vector<double> Results(Np * Ncos * Nphi);
    ...
    for (size_t i = 0; i < Np; i++) {
        ...
        for (size_t j = 0; j < Ncos; j++) {
            ...
            for (size_t k = 0; k < Nphi; k++) {
                double phiMin = HatFunctionBasis::PhiVals[
                                    (k == 0) ? 0 : k - 1];
                double phiMax = HatFunctionBasis::PhiVals[
                                    (k == Nphi - 1) ? Nphi - 1 : k + 1];
                GSLARGS args;
                args.indexP = i;
                ...
                Results[k + j * Nphi + i * Ncos * Nphi] = result;
...
inline double CollisionIntegral(double *x, const GSLARGS& args) {
    ...
    double M4 = Traits::matrix(s, t, u, q23, q13, mDSqr, mQSqr);
...
void Compute(std::string OutputFile) {
    ...
    size_t Ncos = HatFunctionBasis::CosThetaParams.Nx;
    size_t Nphi = HatFunctionBasis::PhiParams.Nx;
    std::vector<double> Results(Np * Ncos * Nphi), Errors(Np * Ncos * Nphi);
    ...
    for (size_t i = 0; i < Np; i++) {
        for (size_t j = 0; j < Ncos; j++) {
            for (size_t k = 0; k < Nphi; k++) {
                ...
                Results[k + j * Nphi + i * Ncos * Nphi] = result;
                Errors[k + j * Nphi + i * Ncos * Nphi] = error;
                #pragma omp critical
                {
                    if (i % (Np / 4) == 0 && j == 0) {
                        fmt::println(stderr,
    ...
    file.print("# p cosTheta dist C_i C_iErr Area\n");

    for (size_t k = 0; k < Nphi; k++) {
        file.print("# Phi: {}\n", HatFunctionBasis::PhiVals[k]);
        ...
        for (size_t i = 0; i < Np; i++) {
            for (size_t j = 0; j < Ncos; j++) {
                file.print("{} {} {} {} {} {}\n",
                           ...
                           Traits::dist1(HatFunctionBasis::MomentumVals[i],
                                         HatFunctionBasis::CosThetaVals[j],
                                         HatFunctionBasis::PhiVals[k]),
                           Results[k + j * Nphi + i * Ncos * Nphi],
                           Errors[k + j * Nphi + i * Ncos * Nphi],
                           HatFunctionBasis::MomentumArea[i] *
                           HatFunctionBasis::CosThetaArea[j] *
                           HatFunctionBasis::PhiArea[k]);
            }
