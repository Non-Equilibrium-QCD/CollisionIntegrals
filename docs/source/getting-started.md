...
:::{literalinclude} ../../examples/YM.cpp
:language: c++
:caption: Example program computing the gluon-gluon scattering collision integral.
:linenos:
:::

This program is written in `examples/YM.cpp`. Build and run it with:
```bash
make example-run NAME=YM
```
It will compute the collision integral for gluon-gluon scattering and output the result to `OUTPUT/QCDgg_gg.dat`.
